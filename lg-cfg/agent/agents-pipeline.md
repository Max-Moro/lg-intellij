...
2. Запустить агента @linter (без параметров - ktlint обрабатывает весь проект автоматически)
...
Данный агент занимается вызовом линтера и исправлением возникших проблем.

**Важно:** ktlint Gradle Plugin не поддерживает проверку конкретных файлов - он всегда обрабатывает весь проект. Это design limitation инструмента, не баг. Поэтому агенту не нужно передавать список файлов. 
...
2. **ОБЯЗАТЕЛЬНО** запустить агента @linter (проверит весь проект автоматически).
