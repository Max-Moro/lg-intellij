...
<!-- lg:else -->
This project currently uses only the 2 agents mentioned above.
<!-- lg:endif -->
...
4. Code verification:
<!-- lg:if tag:code-inspector -->
   - Perform code quality check by calling the @code-inspector agent.
<!-- lg:endif -->
   - Perform compilation of the codebase by calling the @compiler agent.
5. Receive all reports from subagents and resolve their problems if any arise.
6. Create a final brief summary for the user on the results of the iteration work and send it to the dialog.
7. Complete the work and wait for user feedback and/or testing results from the user.
...
2. Code verification:
<!-- lg:if tag:code-inspector -->
   - **MANDATORY** run the @code-inspector agent to check code quality.
<!-- lg:endif -->
   - **MANDATORY** run the @compiler agent to check the build.
3. Receive all reports from subagents and resolve their problems if any arise.
4. Quickly respond to the user that the fix is ready for testing (visual check). A comprehensive summary is not required in this case.

<!-- lg:if tag:code-inspector -->
**Important:** Even in a short cycle, you cannot skip code inspection and compilation checks.
<!-- lg:else -->
**Important:** Even in a short cycle, you cannot skip compilation checks.
<!-- lg:endif -->
...
<!-- lg:else -->
If the @compiler subagent returns a status of "⚠️ Attention Required" or "⚠️ Manual Review Needed":
<!-- lg:endif -->
